...
# Sidebar Configuration for Multi-Provider Support
with st.sidebar:
    st.subheader("Model Provider")
    provider = st.selectbox("Select Provider", ["Gemini", "OpenAI", "Claude"])
    
    # Default API keys from environment variables / secrets
    default_key = ""
    if provider == "Gemini":
        default_key = os.environ.get("GEMINI_API_KEY", "")
        model_options = ["gemini-3.7-flash", "gemini-3.1-pro-preview"]
    ...
    user_api_key = st.text_input(f"{provider} API Key", value=default_key, type="password")
...
# Reset session if provider or model changes
        ...
        with st.spinner(f"Generating via {provider}..."):
            ...
            try:
                if provider == "Gemini":
                    ...
                    # Re-send full transcript history to maintain context
                    for m in st.session_state.messages[:-1]:
                        if m["role"] == "user":
                            pass  # Handled seamlessly via loop or session state
                    ...
                    anthropic_client = anthropic.Anthropic(api_key=user_api_key)
                    formatted_messages = []
                    for m in st.session_state.messages:
                        formatted_messages.append({"role": m["role"], "content": m["content"]})
