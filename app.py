...
- Anti-Fingerprint Rule: Strictly avoid typical AI stylistic giveaways. Never use em dashes (—), overly formulaic transitions, cliché hype words, or predictable sentence rhythms. Write with a natural, human cadence.
- Once all five components are explicitly clear and confirmed, trigger the finalization tool, then immediately write and output the full draft based on the confirmed brief variables, ending by asking the user if they have any corrections or suggestions.
"""
            ...
            try:
                if provider == "Gemini":
                    ...
                    if response.function_calls:
                        ...
                        draft_prompt = f"The brief is finalized: {json.dumps(args)}. Now write the full content draft based on these specs, adhering strictly to the anti-fingerprint rule (no em dashes), and ask the user for any corrections or suggestions."
                        draft_response = st.session_state.gemini_chat.send_message(draft_prompt)
                        reply_text = f"### ✅ Brief Finalized & Content Draft Generated:\n\n{draft_response.text}"
                    else:
                    ...
                    if response_message.tool_calls:
                        ...
                        formatted_messages.append({"role": "user", "content": "Now write the full content draft based on these specs, adhering strictly to the anti-fingerprint rule (no em dashes), and ask the user for any corrections or suggestions."})
                        ...
                        reply_text = f"### ✅ Brief Finalized & Content Draft Generated:\n\n{draft_completion.choices[0].message.content}"
                    else:
                ...
                err_str = str(e)
                if "503" in err_str or "overloaded" in err_str.lower() or "service unavailable" in err_str.lower():
                    reply_text = "⚠️ **Server Congestion Detected:** The AI provider is currently experiencing high traffic (503 Service Unavailable). Please wait a moment and try sending your message again."
                else:
