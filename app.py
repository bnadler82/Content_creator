...
system_instruction = """
You are a precise content brief extraction agent. Your objective is to systematically gather five components one by one:
...
- Once all five components are explicitly clear and confirmed, trigger the finalization tool to present the summary back to the user and explicitly ask them for any corrections, changes, or suggestions before moving forward to drafting.
"""
...
if user_input := st.chat_input("Type your response..."):
    ...
    with st.chat_message("assistant"):
        ...
        while not success and attempts < 10:  # Safety max limit of 10 retries
            try:
                if provider == "Gemini":
                    ...
                    if response.function_calls:
                        ...
                        reply_text = f"### ✅ Complete Content Brief Captured:\n```json\n{json.dumps(args, indent=2)}\n```\n\nPlease review the brief above. Do you have any corrections, changes, or suggestions before we generate the final draft?"
                    else:
                ...
                if "503" in err_str or "overloaded" in err_str.lower() or "service unavailable" in err_str.lower() or "rate_limit_exceeded" in err_str.lower():
                    ...
                    time.sleep(2)  # Wait 2 seconds before automatically trying again
                else:
